...
class SharedwallMethodCache {
    ...
    public synchronized long rebuild() {
        ...
        for (Object bean : cellBeans.values()) {
            Object nonNullBean = Objects.requireNonNull(bean, "cell bean must not be null");
            Class<?> cls = AopUtils.getTargetClass(nonNullBean);
            List<MethodCandidate> candidates = findSharedCandidates(nonNullBean, cls);
