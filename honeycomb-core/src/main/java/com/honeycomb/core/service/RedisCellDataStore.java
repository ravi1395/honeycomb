...
import com.honeycomb.core.config.HoneycombStorageProperties;
...
public class RedisCellDataStore implements CellDataStore {
    ...
    public Flux<Map<String,Object>> list(String cell) {
        ...
        return redisTemplate.keys(pattern)
                .flatMap(key -> redisTemplate.opsForValue().get(Objects.requireNonNull(key, "redis key must not be null")))
                .flatMap(this::deserialize);
    }

    @Override
    public Mono<Map<String,Object>> get(String cell, String id) {
        return redisTemplate.opsForValue().get(Objects.requireNonNull(key(cell, id), "redis key must not be null"))
                .flatMap(this::deserialize);
    }
    ...
    public Mono<Map<String,Object>> create(String cell, Map<String,Object> payload) {
        ...
        return serializePayload(id, payload)
                .flatMap(json -> {
                    String nonNullJson = Objects.requireNonNull(json, "serialized payload must not be null");
                    String nonNullKey = Objects.requireNonNull(key(cell, id), "redis key must not be null");
                    return redisTemplate.opsForValue().set(nonNullKey, nonNullJson).thenReturn(nonNullJson);
                })
                .flatMap(this::deserialize);
    }
    ...
    public Mono<Map<String,Object>> update(String cell, String id, Map<String,Object> payload) {
        String key = Objects.requireNonNull(key(cell, id), "redis key must not be null");
        return redisTemplate.opsForValue().get(key)
                .switchIfEmpty(Mono.empty())
                .flatMap(existing -> serializePayload(id, payload)
                        .flatMap(json -> {
                            String nonNullJson = Objects.requireNonNull(json, "serialized payload must not be null");
                            return redisTemplate.opsForValue().set(key, nonNullJson).thenReturn(nonNullJson);
                        })
                        .flatMap(this::deserialize));
    }
